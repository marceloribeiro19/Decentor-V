...
float node_loss_pd(loss_functions loss, float output_trg, float model_out)
{
    ...
    // Proteção geral para valores muito próximos de 0 ou 1 (evitar divisões por zero e instabilidades)
    ...
    case BCE:
        // Evitar valores extremos que podem causar divisão por zero
        if (model_out <= epsilon)
            model_out = epsilon;
        else if (model_out >= 1.0f - epsilon)
            model_out = 1.0f - epsilon;

        num = model_out - output_trg;
        den = model_out * (1.0f - model_out);

        loss_derivative = num / den;
        break;

    case CE:
        // Evitar divisão por zero se model_out for zero
		if (model_out < 0.001)
		model_out = 0.001;
...
float node_activation_pd(const float node_output, activation_function_type act_func)
{
	...
	case SIGMOID:
			one_minus_output = 1.0f - node_output;
			activation_pd = one_minus_output * node_output;
		break;
