...
int8_t __attribute__((always_inline)) pulp_nn_bn_quant_int8 (
  int32_t phi,
  int32_t k,
  int32_t lambda,
  int8_t  d
  ...
  int32_t integer_image_phi = (k * phi) + lambda;
  int32_t x = (integer_image_phi) >> d;
  int8_t res = (int8_t)__builtin_pulp_clipu_r(x, 127);
  return res;
}
...
int8_t __attribute__((always_inline)) pulp_nn_quant_int8(
  int32_t phi,
  int16_t m,
  int8_t  d
) {
